...
class MysqliDb
{
     ...
     */
    public function __construct($host = DB_HOST, $username = DB_USER, $password = DB_PASS, $db = DB_NAME, $port = null, $charset = 'utf8')
    {
        $isSubQuery = false;
     ...
     */
    public function get($tableName, $numRows = null, $columns = '*',$cond="")
    {
        if (empty($columns)) {
            $columns = '*';
        }
        ...
        $this->_query = 'SELECT ' . implode(' ', $this->_queryOptions) . ' ' .
            $column . " FROM " . $this->_tableName." $cond";
        $stmt = $this->_buildQuery($numRows);
